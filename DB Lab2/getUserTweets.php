...
//     var_dump($foundUser);
    ...
    //Everythng below this line is to test collecting multiple searches and adding them into separate tables of the database 
    //so we can later compare them. The code is literally copy and pasted from above with just a minor change to the variable
    //$userName. $userName in this function is now $userName2 and the userString is now userString2.
    require_once('oauth/twitteroauth/twitteroauth.php'); // https://github.com/abraham/twitteroauth
    $userName2 = $_GET['userString2']; // get the searched for user name
    // don't be a dick by stealing my keys
    // We'll define the key values in case we need to share them across functions in the future
    define('CONSUMER_KEY', 'aqEWrdsZtvnWskYXZe0Ui1dAs');
    define('CONSUMER_SECRET', 'VCtIhC9mUrAOlljEIhPJ98msypf5WgNofTxt0F7COFYohUiFZt');
    ...
    function getTweets($userName2) { 
        ...
        $connection = connectToTwitter();
        // hopefully connection worked
        $tweets = $connection->get("https://api.twitter.com/1.1/statuses/user_timeline.json?screen_name=".$userName2."&count=".$returnedTweetsLimit); // get the tweets for the specified user
         
        return ($tweets);
    }

    echo "<p><a href=\"home.html\"><- Return home</a></p>";
    echo "<p>Tweets by <a href=\"http://twitter.com/".$userName2."\">@" . $userName2 . "</a></p>";
    echo "<p><strong>Note:</strong> If you see the message 'Notice: Trying to get property of non-object in...' you are probably looking for a user that doesn't exist. Please try again.</p>";
    $tweets = getTweets($userName2); // call the function to fetch user tweets
    foreach ($tweets as $line) { // step through each returned tweet
        $status = $line->text; // strip the Tweet from the JSON
        $tweetTime =  $line->created_at; // strip creation time from the JSON
        $tweetId = $line->id_str; // strip the tweet ID so we can link back to the source tweet
        $outputTweet = '<li>'.$status.'</span> <a style="font-size:85%" href="http://twitter.com/'.$userName2.'/statuses/'.$tweetId.'">'. $tweetTime .'</a></li>'; // Render our beautiful new tweet
        echo $outputTweet; // echo the tweet
    }
?>
</body>
